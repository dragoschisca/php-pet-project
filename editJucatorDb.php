...
if ($uploadOk == 0) {
  ...
  if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
    try
	{
		...
		$sql="UPDATE jucatori SET Nume='".$_POST['nume']."', Prenume='".$_POST['prenume']."', Club='".$_POST['club']."', Pozitia='".$_POST['pozitia']."', Varsta='".$_POST['varsta']."', Poza='".$target_file."' WHERE Id=".$_POST["Id"];
		$conn->query($sql);
...
else
{
  try
  {
    $conn=new mysqli($host,$user,$pas,$dbname);
    echo "Conexiunea este reușită<br>";
    $sql="UPDATE jucatori SET Nume='".$_POST['nume']."', Prenume='".$_POST['prenume']."', Club='".$_POST['club']."', Pozitia='".$_POST['pozitia']."', Varsta='".$_POST['varsta']."'";

    if (!empty($target_file)) {
        $sql .= ", Poza='".$target_file."'";
    }
    
    $sql .= " WHERE Id=".$_POST["Id"];
    
    $conn->query($sql);
    echo "Datele au fost modificate cu succes<br>";
    $conn->close();
  }
  catch(Exception $error)
  {
