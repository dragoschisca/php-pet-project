...
if ($uploadOk == 0) {
  ...
  if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
    try
	{
		...
		$sql="UPDATE users SET username='".$_POST['username']."', email='".$_POST['email']."', password='".$_POST['password']."', Poza='".$target_file."' WHERE id=".$_POST["id"];
		$conn->query($sql);
...
else
{
  try
  {
    $conn=new mysqli($host,$user,$pas,$dbname);
    echo "Conexiunea este reușită<br>";
    $sql="UPDATE users SET username='".$_POST['username']."', email='".$_POST['email']."', password='".$_POST['password']."'";
